...
export const NearbyRestaurants: React.FC<NearbyRestaurantsProps> = ({
  ...
  // 근처 맛집 게시물 로드 (소수점 둘째 자리까지 같은 위치)
  useEffect(() => {
    const loadNearbyRestaurants = async () => {
      try {
        ...
        // 모든 게시물 가져오기 (더 넓은 범위)
        // getPostsByLocation은 정확한 위치만 찾으므로 직접 필터링
        const { getPosts } = await import('../../services/postService');
        const allPosts = await getPosts(100); // 최근 100개 게시물
        
        // 소수점 둘째 자리까지 같은 위치의 게시물 필터링
        // 단, 정확히 같은 위치는 제외 (이미 "이 장소와 같은 게시물"에 표시됨)
        const nearbyPosts = allPosts.filter(post => {
          ...
          // 정확히 같은 위치는 제외
          if (postLat === lat && postLng === lng) return false;
          
          // 소수점 둘째 자리까지 비교
          const postRoundedLat = Math.round(postLat * 100) / 100;
          const postRoundedLng = Math.round(postLng * 100) / 100;
          
          return postRoundedLat === roundedLat && postRoundedLng === roundedLng;
        });
        ...
        // 사용자 정보 로드
        const userIds = [...new Set(nearbyPosts.map(post => post.userId))];
  ...
  if (isLoading) {
    return (
      <div className={styles.container}>
        <h2 className={styles.title}>{t('nearbyRestaurants')}</h2>
  ...
  if (posts.length === 0) {
    return null; // 근처 맛집이 없으면 섹션 자체를 표시하지 않음
  }
